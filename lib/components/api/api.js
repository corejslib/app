...
import Acl from "#lib/api/components/acl";
import Sessions from "#lib/api/components/sessions";
import Tokens from "#lib/api/components/tokens";
import Users from "#lib/api/components/users";
...
export default class AppApi extends Api {
    ...
    #avatarCacheControl;
    #authorizedSessionMaxAgeInterval;
    ...
    get avatarCacheControl () {
        this.#avatarCacheControl ??= "public, max-age=" + this.avatarMaxAgeInterval.toSeconds().rounded.number;

        return this.#avatarCacheControl;
    }

    get authorizedSessionMaxAgeInterval () {
        this.#authorizedSessionMaxAgeInterval ??= Interval.new( this.config.authorizedSessionMaxAge );

        return this.#authorizedSessionMaxAgeInterval;
    }
    ...
    async #downloadAvatar ( req ) {
        const userId = req.path.slice( this.config.avatarUrl.length );
