...
export default class Backup {
    ...
    async #parseBackupLabel ( backupDir ) {
        ...
        if ( await pathExists( path.join( backupDir, "base.tar.gz" ) ) ) {
            var extracted = true;
        ...
        for ( let line of content.split( "\n" ) ) {
            ...
            const match = line.match( /^(?<key>[^:]+): +(?<value>[^ ].*)$/v ),
                key = match.groups.key,
                value = match.groups.value;

            if ( /^(?:START|STOP) WAL LOCATION$/v.test( key ) ) {
                const prefix = key.startsWith( "START" )
                        ? "START"
                        : "STOP",
                    match = value.match( /^(?<lsn>[^ ]+) \(file (?<file>[^\)]+)\)$/v );
