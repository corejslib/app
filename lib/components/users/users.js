import PwnedPasswords from "#lib/api/pwned-passwords";
import constants from "#lib/app/constants";
import User from "#lib/app/user";
import CacheLru from "#lib/cache/lru";
import Passwords from "#lib/crypto/passwords";
...
export default class Users {
    ...
    async validatePassword ( password ) {
        const data = Passwords.default.checkPassword( password );

        if ( !data.isStrong && !this.config.allowWeakPasswords ) {
            return result( [ 400, "Password is weak" ], data );
        }
        else {
            if ( !this.#config.allowPwnedPasswords ) {
                const res = await PwnedPasswords.checkPassword( password );

                if ( !res.ok ) {
                    return res;
                }
                else if ( res.data.pwned ) {
                    return result( [ 400, "Password is pwned" ] );
                }
            }

            return result( [ 200, "Password is ok" ], data );
        }
    }

    generateRandomPassword () {
        return Passwords.default.generateRandomPassword().password;
    }
    ...
    async createUser ( email, { password, enabled, locale, emailConfirmed, root, dbh } = {} ) {
        ...
        // validate password
            ...
            const res = await this.validatePassword( password );
        ...
        const res = await this.app.crypto.passwordHash.createPasswordHash( password );
        if ( !res.ok ) return res;
        const passwordHash = res.data.phc;
    ...
    async setUserPassword ( userId, password, { dbh, validatePassword = true } = {} ) {
        dbh ||= this.dbh;
        ...
        // generate password hash
        let res = await this.app.crypto.passwordHash.createPasswordHash( password );
        if ( !res.ok ) return res;
        const passwordHash = res.data.phc;

        res = await dbh.do( SQL.upsertPasswordHash, [ user.id, passwordHash ] );
