...
export default class NginxProxyUpstreams {
    ...
    #set ( upstreams ) {
        ...
        upstreams = upstreams.map( upstream => this.#validateUpstreamServer( upstream ) ).filter( upstream => upstream );
    ...
    #validateUpstreamServer ( server ) {
        ...
        if ( server instanceof IpAddress ) {
            return `${ server }:${ this.#proxy.upstreamPort }`;
        }
        ...
        // ip address
            ...
            var [ address, port ] = server.split( ":", 2 );
