...
import { decrypt, encrypt, getCipherInfo, hash } from "#core/crypto";
import RandomValues from "#core/crypto/random-values";
import { fromMsgPackStream, toMsgPack } from "#core/msgpack";
import Mutex from "#core/threads/mutex";

const MASTER_KEY_ID = -1;

export default class CryptoKeyring {
    #crypto;
    #masterKey;
    #cipherInfo;
    #keys = {};
    ...
    async revokeKey () {
        ...
        const sharedMutex = this.#getSharedMutex( "lock" ),
    ...
    #parseKey ( key ) {
        try {
            if ( Buffer.isBuffer( key ) || typeof key === "string" ) {
                key = crypto.createSecretKey( key, "base64" );
            }
    ...
    async #createKey () {
        ...
        try {
            ...
            res = result.fromError( e, { "log": false } );
        }
    ...
    async #initEncryption ( { buffer, key } = {} ) {
        ...
        if ( buffer ) {
            ...
            const res = fromMsgPackStream( buffer );
        ...
        // create header
            ...
            // generate iv
            const iv = Buffer.from( RandomValues.default.getRandomArrayBuffer( this.#cipherInfo.ivLength ) );

            return {
                "algorithm": this.#cipherInfo.algorithm,
                "key": key.key,
                iv,
                "header": toMsgPack( [ this.#cipherInfo.algorithm, key.id, iv ] ),
            };
        }
    }
