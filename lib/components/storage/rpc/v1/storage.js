export default Super =>
    class extends Super {
        ...
        // public
        async [ "API_download" ] ( ctx, path ) {
            const message = await this.app.storage.downloadFile( path );

            return result( 200, message );
        }

        async [ "API_upload" ] ( ctx, path, message, options ) {
            return this.app.storage.upload( path, message, options );
        }
