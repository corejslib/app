...
CREATE FUNCTION create_acl ( p_acl_type text ) RETURNS int53 AS $$
BEGIN

    RETURN create_acl( ( SELECT id FROM acl_type WHERE type = p_acl_type ) );
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION create_acl ( p_acl_type_id int53 ) RETURNS int53 AS $$
DECLARE
    v_id int53;
BEGIN
    INSERT INTO acl ( acl_type_id ) VALUES ( p_acl_type_id ) RETURNING id INTO v_id;

    RETURN v_id;
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION get_acl_users ( p_acl_id int53 ) RETURNS json AS $$
BEGIN
    RETURN ( SELECT json_agg( acl_user.user_id ) FROM acl_user WHERE acl_user.acl_id = p_acl_id AND acl_user.enabled );
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION acl_user_roles ( p_acl_id int53, p_user_id int53 ) RETURNS json STABLE AS $$
BEGIN

    -- root user
    IF p_user_id = ${ constants.rootUserId } THEN
        RETURN (
            SELECT
                json_agg( role )
            FROM (
        ...
        RETURN (
            ...
            FROM (
                ...
                FROM
                    ...
                    acl_role
                WHERE
                    acl_user_role.acl_id = p_acl_id
                    AND acl_user_role.user_id = p_user_id
...
CREATE FUNCTION acl_user_permissions ( p_acl_id int53, p_user_id int53 ) RETURNS json STABLE AS $$
BEGIN
    ...
    RETURN (
        SELECT
            json_agg( permission )
        FROM
            _get_acl_user_permissions( p_acl_id, p_user_id )
    );

END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION acl_has_user ( p_acl_id int53, p_user_id int53 ) RETURNS boolean STABLE AS $$
BEGIN
    IF p_user_id = ${ constants.rootUserId } THEN
        RETURN EXISTS ( SELECT FROM acl WHERE id = p_acl_id );
    ELSE
        RETURN EXISTS ( SELECT FROM acl_user WHERE acl_id = p_acl_id AND user_id = p_user_id AND enabled );
    END IF;

END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION acl_user_editable ( p_acl_id int53, p_acl_user_id int53, p_parent_user_id int53 ) RETURNS boolean STABLE AS $$
BEGIN

    IF p_parent_user_id = ${ constants.rootUserId } THEN
        RETURN TRUE;
    ELSIF p_acl_user_id = p_parent_user_id THEN
        RETURN FALSE;
    ELSE
        RETURN NOT EXISTS (
            WITH acl_user_permissions AS (
                SELECT permission FROM _get_acl_user_permissions( p_acl_id, p_acl_user_id )
            )
            SELECT FROM
                acl_user_permissions
            WHERE
                permission NOT IN ( SELECT permission FROM _get_acl_user_permissions( p_acl_id, p_parent_user_id ) )
        );
    END IF;
...
CREATE FUNCTION _get_acl_user_permissions ( p_acl_id int53, p_user_id int53 ) RETURNS TABLE ( permission text ) STABLE AS $$
BEGIN
    ...
    ELSE
        RETURN QUERY
            ...
            WHERE
                ...
                AND acl_user.acl_id = p_acl_id AND acl_user.user_id = p_user_id AND acl_user.enabled
            ORDER BY
                acl_permission.permission ASC;
    END IF;
...
BEGIN
    ...
    FOR v_row IN SELECT acl_id, user_id FROM new_table GROUP BY acl_id, user_id LOOP
...
BEGIN
    ...
    FOR v_row IN SELECT acl_id, user_id FROM old_table GROUP BY acl_id, user_id LOOP

        -- acl user was not removed
        IF EXISTS ( SELECT FROM acl_user WHERE acl_user.acl_id = v_row.acl_id AND acl_user.user_id = v_row.user_id ) THEN
