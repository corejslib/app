...
export default class User {
    ...
    async verifyPassword ( password ) {
        ...
        if ( this.#password ) {
            const hash1 = ( this.#hash ||= await hash( COMPARE_HASH, this.#password ) ),
                hash2 = await hash( COMPARE_HASH, password );

            return timingSafeEqual( hash1, hash2 );
        }

        const res = await this.app.crypto.passwordHash.verifyPasswordHash( this.#passwordHash, password );
        if ( res.ok ) {
