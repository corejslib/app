...
export default class Component {
    ...
    async install () {
        try {
            ...
            // lock destroy
            if ( this._destroy ) this.#destroyLock = process.destroyController.lock( this.#id );
    ...
    async destroy () {
        if ( !this.#destroyLock || this.#destroyLock.isDone ) return;

        console.info( `Destroying component: ${ this.id }` );
    ...
    _mergeSubConfig ( location ) {
        ...
        const subConfig = readConfigSync( path );
    ...
    _mergeSubSchema ( schema, location ) {
        ...
        var subSchema = readConfigSync( path );
