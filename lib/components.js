...
import { isKebabCase, toCamelCase } from "#core/naming-conventions";
...
export default class Components {
    ...
    async create ( publicConfig ) {
        ...
        for ( const spec of Object.values( this.#components ) ) {
            try {
                ...
                if ( spec.config.templates ) {
                    templates[ spec.id ] = spec.config.templates;
                }
        ...
        for ( const component of Object.values( this.#components ) ) {
            if ( templates[ component.id ] ) {
                this.app.templates.add( templates[ component.id ] );
            }
        }
    ...
    #loadComponents ( appConfiglocation, registry ) {
        ...
        for ( const dirent of fs.readdirSync( componentsLocation, { "withFileTypes": true } ) ) {
            ...
            const componentId = toCamelCase( componentName ),
    ...
    #validateComponentConfig ( component, schema, config ) {
        if ( !this.#ajvCache[ component.id ] ) {
            this.#ajvCache[ component.id ] = {};

            const schemaPath = component.location + "/config.schema.yaml";

            if ( fs.existsSync( schemaPath ) ) {
                try {
                    const schema = component.applySubSchema( readConfigSync( schemaPath ) );

                    const ajv = new Ajv( {
                        "coerceTypes": false,
                    } ).addSchema( schema );

                    this.#ajvCache[ component.id ].config = ajv;
                    ...
                    if ( ajv.getSchema( "env" ) ) {
                        ...
                        this.#ajvCache[ component.id ].env = ajv;
                    }
                }
                catch ( e ) {
                    return result( [ 500, `Failed to compile JSON schema: ${ e.message }` ] );
                }
            }
        }

        const ajv = this.#ajvCache[ component.id ];
        ...
        try {
            if ( schema ) {
                ...
                // validate config
                if ( ajv?.config?.getSchema( "config" ) && !ajv.config.validate( "config", component.config ) ) {
