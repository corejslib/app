...
`.prepare(),
    ...
    "insertHash": sql`INSERT INTO api_session_hash ( api_session_id, hash ) VALUES ( ?, ? )`.prepare(),

    "updatePublic": sql`UPDATE api_session SET public = ? WHERE id = ?`.prepare(),
...
export default class extends Component {
    ...
    async createSession ( userId, { hostname, userAgent, remoteAddress, dbh } = {} ) {
        ...
        var res = await dbh.begin( async dbh => {
            const expires = this.#sessionMxaxAge.addDate();
            ...
            // generate token
            const token = await Token.generate( this.app, constants.sessionToken.id, id, { "length": constants.sessionToken.length } );
