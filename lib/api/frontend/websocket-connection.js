...
import ServerWebSocketConnection from "#core/http/server/websocket-connection";
...
export default class WebSocketConnection extends ServerWebSocketConnection {
    ...
    #onSubscribe ( names ) {
        ...
        for ( const name of names ) {
            ...
            // already subscribed
            if ( this.#events.hasListeners( name ) ) continue;
    ...
    #subscriber ( name ) {
        ...
        if ( this.api.isRpc ) {
            ...
            if ( ctx.user ) {
                names.push( `${ name }/users`, `${ name }/${ ctx.user.id }` );
