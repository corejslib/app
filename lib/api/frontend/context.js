...
export default class Context {
    ...
    get isEnabled () {
        if ( this.#token ) {
            ...
            if ( !this.#telegramBotUser.isEnabled ) return false;
        }
    ...
    async update () {
        ...
        // update telegram bot user
            ...
            const telegramBot = this.app.telegram.bots.getBotById( this.#telegramBotUser.bot.id );
            ...
            const telegramBotUser = await telegramBot.users.getTelegramUserById( this.#telegramBotUser.id );
            ...
            // telegram bot user was unsubscribed
            else if ( !telegramBotUser?.isSubscribed ) {
                this.#isDeleted = true;
    ...
    async #call ( methodId, args, isVoidCall ) {
        ...
        if ( typeof methodId === "object" ) {
            ( { "method": methodId, args, signal } = methodId );
        }
        ...
        const api = this.#api,
            ...
            isPrivateCall = this.#method
                ? true
                : this.#isPrivateCall;

        // reject public api calls if frontend is destroying
        if ( frontend.isDestroying && !isPrivateCall ) {
            return result( -32_816 );
        }

        // context is deleted
        if ( this.#isDeleted ) return result( -32_815 );
        ...
        // method not found
        if ( !method ) return result( -32_809 );

        // method not implemented
        if ( typeof method.object[ method.apiName ] !== "function" ) return result( -32_809 );

        // check connection type
        if ( method.connectionType ) {

            // websockets connection
            if ( this.#connection ) {
                if ( method.connectionType !== "websockets" ) return result( -32_810 );
            }

            // http connection
            else {
                if ( method.connectionType !== "http" ) return result( -32_810 );
            }
        }

        // start call
        // increment activity counter for non long-running methods
        const callDescriptor = frontend.startApiCall( {
            "activityCounter": method.connectionType !== "websockets",
        } );
        ...
        try {
            ...
            if ( this.#token || this.#telegramBotUser ) {
                ...
                // api backend is down
                if ( !api.isConnected ) throw result( -32_814 );

                // unable to update context, api backend is down
                if ( !( await this.update() ) ) throw result( -32_814 );

                // aborted
                if ( signal?.aborted ) throw result( -32_817 );

                // context is deleted
                if ( this.#isDeleted ) throw result( -32_815 );

                // context is disabled
                if ( !this.isEnabled ) throw result( -32_813 );
            }
            ...
            if ( api.isApi ) {
                ...
                // too many requests
                if ( !allowCall ) throw result( -32_802 );
            }
            ...
            if ( api.isApi ) {
                ...
                if ( method.permission ) {
                    if ( method.permission === "telegram-bot-users" ) {
                        if ( !this.#telegramBotUser ) throw result( -32_811 );
                    }
                ...
                if ( method.requireAuthorization && this.#token.isSessionToken && !this.#token.checkAuthorization( this.#remoteAddress, this.#userAgent ) ) {
                    throw result( -32_812 );
                }
            }
            else {
            ...
            // call method
            res = await api.app.monitoring.monitorMethodCall( api.isApi
                ? "api"
                : "rpc", method.id, method.object[ method.apiName ].bind( method.object, ctx, ...args ) );

            // result is Message
            if ( res.data instanceof Message ) {
                if ( method.connectionType !== "http" ) {
                    res.data.destroy();

                    throw result( [ 500, "Invalid API result type. Http message returned on non-http request." ] );
                }

                // destroy message in void call
                else if ( isVoidCall ) {
                    res.data.destroy();
                }
            }
