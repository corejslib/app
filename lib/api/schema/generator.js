...
export default class {
    ...
    async start () {
        ...
        for ( const version in versions ) {
            fileTree.add( new File( {
                "path": `${ version }.md`,
                "sources": await ejs.renderFile( apiSchemaTmpl, {
                    "generator": this,
                    version,
    ...
    async getMethodParamDescription ( method, param ) {
        ...
        // attachment keyword
        else if ( param.schema.attachment ) {
            desc += await attachmentKeyword.getDescription( param );
        }
        ...
        // indent
        if ( desc ) desc = " " + desc.replaceAll( /^/gmv, " ".repeat( 4 ) ).replaceAll( /^ +$/gmv, "" ).trim();
