...
export default class ApiMethod {
    ...
    get maxAttachmentContentLength () {
        return this.#maxAttachmentContentLength;
    }
    ...
    setConfig ( config ) {
        ...
        if ( Reflect.has( config, "connectionType" ) ) {
    ...
    validateParams ( params ) {
        try {
            aclResolverKeyword.clearAclResolvers();

            const ok = this.#paramsValidator( params );

            const aclResolvers = aclResolverKeyword.clearAclResolvers();
    ...
    #buildParamsValidator () {
        ...
        try {
            aclResolverKeyword.clearAclResolverSchemas();
            attachmentKeyword.clearMaxContentLength();

            this.#paramsValidator = buildParamsValidator( paramsSchema, false );

            this.#aclResolvers = aclResolverKeyword.clearAclResolverSchemas();
            this.#maxAttachmentContentLength = attachmentKeyword.clearMaxContentLength();
