...
import JsonContainer from "#core/json-container";
import Message from "#core/message";
import Counter from "#core/threads/counter";
import { resolve } from "#core/utils";
import Component from "#lib/api/component";
import Context from "#lib/api/frontend/context";
import WebSocketConnection from "#lib/api/frontend/websocket-connection";
...
export default class extends Component {
    ...
    #configureHttpServer () {
        const httpServer = this.api.httpServer,
            publishRemoteIncomingEvent = this.#publishRemoteIncomingEvent.bind( this ),
            websocketsConfig = {
                "maxBackpressure": 0,
                ...
                "compress": this.api.webSocketCompress,
    ...
    async #onWebSocketUpgrade ( req ) {
        ...
        const [ protocol, token ] = secWebSocketProtocol?.split( /\s*,\s*/v ) || [];
    ...
    async #onHttpRequest ( req, methodId ) {
        ...
        try {
            voidCall = req.headers.get( "x-api-void-call" ) === "?1";
            ...
            // POST
                ...
                if ( req.headers.contentType.type === "multipart/form-data" ) {
                    ...
                    const method = this.api.schema.methods[ methodId ],
                        maxBufferLength = Math.max( this.api.maxApiRequestBodySize, 1024 ** 2 ), // 1 MiB
                        multipartData = await req.parseMultipartBody( {
                            "maxContentLength": method?.maxAttachmentContentLength || maxBufferLength,
                            maxBufferLength,
                        } );

                    try {
                        const params = multipartData.fields.params.createBodySync();

                        args = JSON.parse( params, ( key, value ) => {

                            // attachment
                            if ( typeof value === "string" && value.startsWith( "attachment:" ) && multipartData.fields[ value ] ) {
                                return multipartData.fields[ value ];
                            }
                            else {
                                return value;
                            }
                ...
                // application/json
                    ...
                    const body = await req.buffer( { "maxLength": this.api.maxApiRequestBodySize } ).catch( e => {
        ...
        const message = res.data instanceof Message
            ? res.data
            : undefined;

        await using disposableStack = message
        ...
        if ( message && res.ok ) {
            ...
            req.end( {
                ...
                body,
                "options": {
                    "compress": this.api.httpCompress,
                },
            } );
        }
    }
    ...
    #decodeArguments ( buffer, contentType ) {
        ...
        if ( contentType.type !== "application/json" ) throw result( -32_803 );
    ...
    async #authenticate ( token, { signal, hostname, userAgent, remoteAddress } = {} ) {
        ...
        // telegram web app token
            ...
            try {
                token = JSON.parse( decodeURIComponent( token ).slice( 9 ) );
            }
            catch {
                return;
            }
        ...
        // api / session token
            ...
            else if ( !internalToken || !internalToken.verify( token ) ) {
                ctx = new Context( this.api, {
                    "isDeleted": true,
                    signal,
                    hostname,
