import Context from "#core/api/frontend/context";
import DigitalSize from "#core/digital-size";
...
export default class Api {
    ...
    get idleTimeout () {
        this.#idleTimeout ??= Interval.new( this.config.frontend.idleTimeout ).toSeconds().rounded.number;
    ...
    get httpCompress () {
        if ( this.#httpCompress === undefined ) {
            this.#httpCompress = this.config.frontend.httpCompress ?? null;

            if ( typeof this.#httpCompress === "string" ) {
                this.#httpCompress = new DigitalSize( this.#httpCompress ).bytes;
            }
        }
    ...
    get webSocketCompress () {
        if ( this.#webSocketCompress === undefined ) {
            this.#webSocketCompress = this.config.frontend.webSocketCompress ?? null;

            if ( typeof this.#webSocketCompress === "string" ) {
                this.#webSocketCompress = new DigitalSize( this.#webSocketCompress ).bytes;
            }
        }
